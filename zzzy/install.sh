...
install_soga() {
    ...
    if  [ $# == 0 ] ;then
        ...
        wget -N --no-check-certificate -O /usr/local/soga.tar.gz https://github.com/Anakindai/crack/raw/main/zzzy/soga-cracked-linux64.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/Anakindai/crack/raw/main/zzzy/soga-cracked-linux64.tar.gz"
    ...
    if [[ ! -f /etc/soga/soga.conf ]]; then
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}soga 可能启动失败，请稍后使用 soga log 查看日志信息，若无法启动，则可能更改了配置格式"
        fi
    fi
    ...
    curl -o /usr/bin/soga -Ls https://github.com/Anakindai/crack/raw/main/zzzy/soga.sh
