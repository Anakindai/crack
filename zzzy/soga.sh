...
install() {
    bash <(curl -Ls https://github.com/Anakindai/crack/raw/main/zzzy/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://github.com/Anakindai/crack/raw/main/zzzy/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/soga -N --no-check-certificate https://github.com/Anakindai/crack/raw/main/zzzy/install.sh
    if [[ $? != 0 ]]; then
        echo ""
